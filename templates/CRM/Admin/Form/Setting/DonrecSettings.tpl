...
<div class="crm-block crm-form-block">
  <div>
    ...
    <input value="all" type="checkbox" id="financial_types_all" name="financial_types_all" {if $financialTypes[0]}checked="checked{/if}" {if count($financialTypes) eq 0}disabled="disabled"{/if} class="form-checkbox"/><label for="financial_types"> {ts domain="de.systopia.donrec"}All deductible contribution types{/ts} <a onclick='CRM.help("{ts domain="de.systopia.donrec"}Contribution Types{/ts}", {literal}{"id":"id-contribution-types","file":"CRM\/Admin\/Form\/Setting\/DonrecSettings"}{/literal}); return false;' href="#" title="{ts domain="de.systopia.donrec"}Help{/ts}" class="helpicon">&nbsp;</a></label>&nbsp;
    <div id="advContribTypeList" {if $financialTypes[0] eq 1}hidden{/if}>
      ...
      <input value="{$item[0]}" type="checkbox" id="financial_types{$smarty.foreach.fitems.iteration}" name="financial_types{$smarty.foreach.fitems.iteration}" {if $item[3]}checked="checked"{/if} class="form-checkbox"/><label for="financial_types"> {$item[1]} {if $item[2]}({ts domain="de.systopia.donrec"}deductible{/ts}){/if}</label>&nbsp;<br/>
      {/if}
    {/foreach}
    </div>
  </div>
  ...
  <div>
    <h3>{ts domain="de.systopia.donrec"}Donation receipts{/ts}</h3>
    <div>
      <table>
        <tr>
