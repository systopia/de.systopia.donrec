...
class CRM_Admin_Page_DonrecProfiles extends CRM_Core_Page {
  ...
  public function run() {
    ...
    foreach ($profiles as $profile_id => $profile_data) {
      ...
      $profiles[$profile_id]['last_used'] = (!is_null($last_used) ? $last_used->format('Y-m-d H:i:s') : E::ts('Never'));
      $profiles[$profile_id]['first_used'] = (!is_null($first_used) ? $first_used->format('Y-m-d H:i:s') : E::ts('Never'));
