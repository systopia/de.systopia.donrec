...
class CRM_Donrec_Form_Task_Create extends CRM_Core_Form {
  ...
  public function setDefaultValues(): array {
    ...
    $this->getElement('cid')->setValue((string) $contactId);
  ...
  public function postProcess(): void {
    ...
    $session->set('url_back', CRM_Utils_System::url(
      'civicrm/contact/view',
      "reset=1&cid=$contactId&selectedChild=donation_receipts"
    ));
    ...
    elseif (empty($result['snapshot'])) {
      CRM_Core_Session::setStatus(
        E::ts('This contact has no selectable contributions in the selected time period.'),
        E::ts('Warning'),
        'warning'
      );
