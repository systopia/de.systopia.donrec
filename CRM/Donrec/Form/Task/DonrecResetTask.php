...
class CRM_Donrec_Form_Task_DonrecResetTask extends CRM_Contact_Form_Task {
  ...
  public function buildQuickForm(): void {
    ...
    if ($duplicate_count) {
      $this->assign(
        'duplicate_warning',
        E::ts('Caution: %1 contact(s) will have more than one donation receipt withdrawn!', [1 => $duplicate_count])
      );
    }
  ...
  public function postProcess(): void {
    if ($this->controller->_actionName[1] == 'next') {
      ...
      /** @var \CRM_Core_DAO $receipt_query */
      ...
      // show message
      CRM_Core_Session::setStatus(E::ts('%1 donation receipts have been withdrawn.', [1 => $success_counter]),
        E::ts('Receipts Withdrawn'),
        'info');
      if ($error_counter) {
        CRM_Core_Session::setStatus(
          E::ts('%1 donation receipts have not been withdrawn and encountered an error.', [1 => $error_counter]),
          E::ts('Error'),
          'error'
        );
      }
  ...
  protected function getDuplicateCount() {
    ...
    return CRM_Core_DAO::singleValueQuery(
      'SELECT COUNT(*) FROM (SELECT COUNT(*) AS count_per_contact '
        . $this->getBaseSQL('GROUP BY entity_id HAVING count_per_contact > 1) temp'));
  }
