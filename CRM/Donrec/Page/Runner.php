...
class CRM_Donrec_Page_Runner extends CRM_Core_Page {
  ...
  public function run() {
    ...
    $parameters['exporters'] = empty($_REQUEST['exporters']) ? ['Dummy'] : explode(',', $_REQUEST['exporters']);
    ...
    if (empty($url_back)) {
      $url_back = CRM_Utils_System::url('civicrm/dashboard', '', FALSE, NULL, TRUE, FALSE, TRUE);
    }
